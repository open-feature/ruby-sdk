...
RSpec.describe OpenFeature::SDK::EventEmitter do
  ...
  describe '#add_handler' do
    ...
    it 'allows multiple handlers for the same event type' do
      handler2 = ->(event_details) { puts "Handler 2" }
  ...
  describe '#remove_handler' do
    let(:handler1) { ->(event_details) { puts "Handler 1" } }
    let(:handler2) { ->(event_details) { puts "Handler 2" } }
    ...
    it 'does nothing if handler is not registered' do
      unregistered_handler = ->(event_details) { puts "Unregistered" }
  ...
  describe '#trigger_event' do
    ...
    it 'continues executing other handlers even if one fails' do
      failing_handler = ->(event_details) { raise 'Handler failed' }
      received_events2 = []
      ...
      expect { event_emitter.trigger_event(OpenFeature::SDK::ProviderEvent::PROVIDER_READY, event_details) }.not_to raise_error
      expect(received_events).to contain_exactly(event_details)
      expect(received_events2).to contain_exactly(event_details)
    end
  end
  ...
  describe '#clear_all_handlers' do
    before do
      OpenFeature::SDK::ProviderEvent::ALL_EVENTS.each do |event_type|
        event_emitter.add_handler(event_type, ->(event_details) { puts "Handler for #{event_type}" })
      end
    end
  ...
  describe 'thread safety' do
    let(:handler) { ->(event_details) { sleep(0.001) } } # Small delay to increase chance of race conditions
    ...
    it 'handles concurrent triggering safely' do
      ...
      counting_handler = lambda do |event_details|
